...
public:
   ...
   * @param clog_manager clog管理器，用于维护redo log
   */
  RC drop(const char *path, const char *name, const char *base_dir, CLogManager *clog_manager);
  ...
  RC update_record(Trx *trx, Record *record, const std::vector<const FieldMeta *> &update_fields, const std::vector<Value> &values);
  ...
  RC create_index(Trx *trx, const char *index_name, const char *attribute_name, char *attribute_names[MAX_NUM],
...
public:
  RC update_record(Record *record, const std::vector<const FieldMeta *> &update_fields, const std::vector<Value> &values);
